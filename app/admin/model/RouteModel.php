...
class RouteModel extends Model
{
    ...
    public function getRoutes($refresh = false)
    {
        ...
        $allRoutes   = [];
        $cacheRoutes = [];
        $lang_array  = array_column(config('lang_list'),'file');
        $langRoutes = [];
...
//            }
            ...
            if(in_array($er['lang'] ?? '',$lang_array) && $er['lang'] != '')
            {
                $route_dir = CMF_ROOT . "data/conf/";
                if (!file_exists($route_dir)) {
                    mkdir($route_dir);
                }
                $route_file = $route_dir . "route".'_'.$er['lang'].".php";

                if (empty($appUrls[$path]['pattern'])) {
                    $langRoutes[$url] = $fullUrl;
                } else {
                    $langRoutes[$url] = [$fullUrl, [], $appUrls[$path]['pattern']];
                }

                file_put_contents($route_file, "<?php\treturn " . stripslashes(var_export($langRoutes, true)) . ";");
            }
            $default_lang = '';
            $lang_prefix = $er['lang'] != '' ? $er['lang'].'_' : $default_lang;
            $url = $lang_prefix . $url;
