...
class PostService
{
    ...
    public function adminPostList($filter, $isPage = false)
    {
        ...
        $join = [
            ...
            ['__PORTAL_POST_LOG__ cj','cj.post_orderNumber=a.id','left']
        ];
        ...
        $field.=",count(cj.id) AS banben";
        $portalPostModel = new PortalPostModel();
        $articles        = $portalPostModel->alias('a')->field($field)
            ->join($join)
            ->where($where)
            //->order('published_time', 'DESC')
            //->order(['id'=>'desc','published_time'=>'desc'])
            ->order(['a.create_time'=>'desc'])
            ->group('cj.post_orderNumber')
            ->paginate(10);
        ...
        */
        ...
        if (empty($categoryId)) {
            ...
            $article = $portalPostModel->alias('post')->field('post.*')
                ->where($where)
                ->find();
           // echo $portalPostModel->getlastsql();exit;
