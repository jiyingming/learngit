...
class PostLogService
{
    ...
    public function adminPostList($filter, $isPage = false)
    {
        ...
        $PortalPostLogModel = new PortalPostLogModel();
        $articles        = $PortalPostLogModel->alias('a')->field($field)
            ->join($join)
            ->where($where)
    ...
    {
        /*
        *柴进修改 把是否发布 post.post_status 注释了
        */
        $PortalPostLogModel = new PortalPostLogModel();
        ...
        if (empty($categoryId)) {
            ...
            $article = $PortalPostLogModel->alias('post')->field('post.*')
                ->where($where)
                ->find();
    ...
    public function publishedPage($pageId)
    {
        ...
        $PortalPostLogModel = new PortalPostLogModel();
        $page            = $PortalPostLogModel
            ->where($where)
            ->find();
       // echo $PortalPostLogModel->getlastsql();exit;
        return $page;
    }
