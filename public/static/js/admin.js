    ...
    if (ajaxForm_list.length) {
        ...
        Wind.use('ajaxForm', 'artDialog', 'noty', 'validate', function () {
            ...
            ajaxForm_list.each(function () {
                if ($(this).hasClass('simditor-form')) {
                    $.validator.prototype.delegate = function(){};
                }
                $(this).validate({
                    //是否在获取焦点时验证
                    //onfocusout : false,
                        ...
                        $form.ajaxSubmit({
                            ...
                            success: function (data, statusText, xhr, $form) {
                                ...
                                function _refresh() {
                                    ...
                                    //预览cj 713
                                    if(data.data['cj_code']==6){

                                        var a = document.createElement('a');
                                        a.setAttribute('href', data.data['edit_url']);
                                        a.setAttribute('target', '_blank');
                                        a.setAttribute('id', data.data['id']);
                                        // 防止反复添加
                                        if(!document.getElementById(data.data['id'])) {
...
function uploadOne(dialog_title, input_selector, filetype, extra_params, app) {
    ...
    openUploadDialog(dialog_title, function (dialog, files) {
        ...
        $(input_selector + '-icon').val('/static/images/icons/1/' + files[0].icon + '.png');
        $(input_selector + '-icon-img').attr('src','/static/images/icons/1/' + files[0].icon + '.png');
...
function uploadMultiFile(dialog_title, container_selector, item_tpl_wrapper_id, filetype, extra_params, app) {
    ...
    openUploadDialog(dialog_title, function (dialog, files) {
        ...
        $.each(files, function (i, item) {
            ...
            itemtpl     = itemtpl.replace(/\{filepath\}/g, item.filepath);
            itemtpl     = itemtpl.replace(/\{name\}/g, item.name);
            itemtpl     = itemtpl.replace(/\{icon\}/g, item.icon);
            html += itemtpl;
        });
        $(container_selector).append(html);
