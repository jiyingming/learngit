...
class Upload
{
    ...
    public function upload()
    {
        ...
        $fileSaveName    = (empty($app) ? '' : $app . '/') . $strDate . '/' . md5(uniqid()) . "." . $strFileExtension;//dump($fileSaveName);exit;
        ...
        // 根据后缀名判断要显示的icon
        $icon_list = config('icon_list');
        $icon = $icon_list[$arrInfo["suffix"]] ?? 'attachment';

        // 把文件上传到oss
        $res = upFileToOss('./upload/' . $arrInfo["file_path"],false);
        if ($res['status'] == 10) {
