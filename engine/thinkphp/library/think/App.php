...
class App
{
    ...
    public static function run(Request $request = null)
    {
        ...
        try {
            ...
            // 未设置调度信息则进行 URL 路由检测
            if (empty($dispatch)) {
                $config['current_lang'] = $request->langset() ?? $config['default_lang'];
                $config['route_config_file'] = [0 => 'route_'.$config['current_lang'],1 => 'route'];
//                dump($config);
//                die;
    ...
    public static function routeCheck($request, array $config)
    {
        ...
        $current_lang = $config['current_lang'] ?? ''; //当前的语言
        $result = false;
        ...
        if ($check) {
            ...
            if (is_file(RUNTIME_PATH . 'route.php')) {
                ...
                foreach ($files as $file) {
                    //echo CMF_ROOT.'data/' . $file . CONF_EXT;
                    if (is_file(CMF_ROOT.'data/conf/' . $file . CONF_EXT)) {
                        //die('333');
                        // 导入路由配置
                        $rules = include CMF_ROOT.'data/conf/' . $file . CONF_EXT;//dump($rules);die;
            ...
            //$result = Route::check($request, $current_lang.'_'.$path, $depr, $config['url_domain_deploy']);
